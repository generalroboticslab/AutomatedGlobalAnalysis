...
if __name__ == "__main__":
    ...
    #data_dir = "./Data/Hodgkin_Huxley_Sim"
    #pattern = "hodgkin_huxley_delaytype-dt_delaysteps-51_stepsize-0.04_noise-0_ntraj-50_trajlen-1100"

    # Initialize an empty array for the first file to set the structure
    #concatenated_array = None

    # List to track loaded file paths to avoid reloading
    #loaded_files = []

    # Iterate over files in the directory
    #for filename in os.listdir(data_dir):
    #    if pattern in filename and filename.endswith(".npy"):
    ...
    #            current_array = np.load(file_path)
                ...
                # Concatenate to the last dimension
    #            if concatenated_array is None:
    #                concatenated_array = current_array
    #            else:
    ...
    #np.save(f"{data_dir}/{pattern}.npy", concatenated_array)

    #data = dataset.load_data()
    #data = dataset.load_data()

    #config_yaml_path = './DelayKoop/Datasets/Configs/tristable_configs_dx.yaml'
    #dataset = GenerateDataset(config_yaml_path)
    #dataset.collect_data()
    ...
    #data = np.concatenate((data0, data1), axis=3)
    #data = np.concatenate((data0, data1), axis=3)
    # save the data
    #np.save(f"{dataset.combined_dir}.npy", data)
    #np.save(f"{dataset.combined_dir}.npy", data)

    print(data.shape)
    print(data.dtype)

    avg_time = np.mean(data[2, 0, -1, :]-data[2, 0, 0, :])
    print(f"Average time: {avg_time}")
    delays = 21

    #plt.figure(figsize=(12, 6))
    #plt.imshow(data[0, ::delays, :, 0], aspect='auto', cmap='seismic')
    #plt.colorbar()
    #plt.show()
    ...
    ax[0, 0].plot(data[2, 0, ::2, 0], data[0, 0, ::2, 0])
    ax[0, 1].plot(data[2, 0, ::2, 0], data[0, delays, ::2, 0])
    ax[1, 0].plot(data[0, 0, ::2, 0], data[0, delays, ::2, 0])
    plt.show()


    fig, ax = plt.subplots(2, 2)
    ax[0, 0].scatter(data[2, 0, ::2, 15], data[0, 0, ::2, 15])
    ax[0, 1].scatter(data[2, 0, ::2, 15], data[0, delays, ::2, 15])
    ax[1, 0].scatter(data[0, 0, ::2, 15], data[0, delays, ::2, 15])
    plt.show()